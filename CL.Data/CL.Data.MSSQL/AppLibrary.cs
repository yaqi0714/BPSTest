...
{
    public class AppLibrary : CL.Data.Interface.IAppLibrary
    {
        ...
        /// <returns></returns>
        public List<CL.Data.Model.AppLibrary> GetPagerData(out string pager, string query = "", string order = "Type,Title", int size = 15, int number = 1, string title = "", string type = "", string address = "")
        {
            ...
            {
                WHERE.AppendFormat("AND Type IN({0}) ", CL.Utility.Tools.GetSqlInString(type));
            }
            ...
            pager = CL.Utility.Tools.GetPagerHtml(count, size, number, query);
            SqlDataReader dataReader = dbHelper.GetDataReader(sql, parList.ToArray());
            List<CL.Data.Model.AppLibrary> List = DataReaderToList(dataReader);
            dataReader.Close();
            return List;
        }
        ...
        /// 查询一个类别下所有记录
        /// </summary>
        public List<CL.Data.Model.AppLibrary> GetAllByType(string types)
        {
            string sql = "SELECT * FROM AppLibrary WHERE Type IN(" + CL.Utility.Tools.GetSqlInString(types) + ")";
            SqlDataReader dataReader = dbHelper.GetDataReader(sql);
            List<CL.Data.Model.AppLibrary> List = DataReaderToList(dataReader);
            dataReader.Close();
            return List;
        ...
        {
            string sql = "DELETE FROM AppLibrary WHERE ID in(" + CL.Utility.Tools.GetSqlInString(idArray) + ")";
